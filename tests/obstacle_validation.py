...
def test_walkway_benchmark(n, half_len, half_width, mode, run=-1, visual=True):
   ...
   # pos_right = ((np.random.random((n, 2)) - 0.5) * 2.0) * np.array([half_width, half_len])
    ...
    #    print(indicess)
        ...
        out_l = len(indicess)
        pos_left = np.transpose([[-half_width]*out_l, state[indicess, 1]])

        state[indicess, 0] = pos_left[:,0]
        state[indicess, 1] = pos_left[:,1]
        ...
        out_r = len(indicess)
        pos_right = np.transpose([[half_width]*out_r, state[indicess, 1]])
        state[indicess, 0] = pos_right[:,0]
        state[indicess, 1] = pos_right[:,1]
...
for layout in layouts :
    ...
    layout_df.to_csv('data_validation/df_{}'.format(layout))
