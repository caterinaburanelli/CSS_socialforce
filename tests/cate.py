...
@pytest.mark.parametrize('mode', ["horizontal"])
def test_walkway_benchmark(n, half_len, half_width, mode, run=-1, visual=False, half_size_single=1.0, distance=10, size_horizontal=30):
   ...
   # pos_right = ((np.random.random((n, 2)) - 0.5) * 2.0) * np.array([half_width, half_len])    
    ...
    pos_left_up = np.transpose(np.array([np.random.uniform(-half_width, half_width, n),np.random.uniform(0, half_len, n)]))
    pos_left_down = np.transpose(np.array([np.random.uniform(-half_width, half_width, n),np.random.uniform(-half_len, 0, n)]))
    pos_right_up = np.transpose(np.array([np.random.uniform(-half_width, half_width, n),np.random.uniform(0, half_len, n)]))
    pos_right_down = np.transpose(np.array([np.random.uniform(-half_width, half_width, n),np.random.uniform(-half_len, 0, n)]))

    right_preference = np.random.choice(a=[0,1], p=[0.2, 0.8], replace=True, size=n)
    left_preference = np.random.choice(a=[0,1], p=[0.8, 0.2], replace=True, size=n)

    pos_right_up = [ [x[0]*y, x[1]*y] for x,y in zip(pos_right_up, right_preference)]
    pos_right_down = [ [ -x[0]*(y-1), -x[1]*(y-1)] for x,y in zip(pos_right_down, right_preference)]
    pos_left_up = [ [x[0]*y, x[1]*y] for x,y in zip(pos_left_up, left_preference)]
    pos_left_down = [ [ -x[0]*(y-1), -x[1]*(y-1)] for x,y in zip(pos_left_down, left_preference)]

    for ind in range(len(pos_right_up)) :
        if (pos_right_up[ind][0] == 0) and (pos_right_up[ind][1] == 0) :
            pos_right_up[ind] = pos_right_down[ind]
    
    for ind in range(len(pos_left_up)) :
        if (pos_left_up[ind][0] == 0) and (pos_left_up[ind][1] == 0) :
            pos_left_up[ind] = pos_left_down[ind]
    
    pos_left = pos_left_up
    pos_right = pos_right_up
    ...
    if (mode=="benchmark"):
        ...
        space = [
            np.array([(x, half_len) for x in np.linspace(-half_width, half_width, num=5000)]),
            np.array([(x, -half_len) for x in np.linspace(-half_width, half_width, num=5000)]),
            np.array([(i, i) for i in np.linspace(half_size_single, -half_size_single)])
        ]
    elif (mode=="pillars"):
        a_values = np.arange(-half_width, half_width, distance)[1:]
        b = 0
        radius = 0.3
        stepSize = 0.1
        t = 0
        ...
        space = [
            np.array([(x, half_len) for x in np.linspace(-half_width, half_width, num=5000)]),
            np.array([(x, -half_len) for x in np.linspace(-half_width, half_width, num=5000)]),
            np.array([(x, 0) for x in np.linspace(-half_width/4, half_width/4, num=5000)])
        ]
    elif (mode=="angled"):
        space = [
            np.array([(x, half_len) for x in np.linspace(-half_width, half_width, num=5000)]),
    ...
    #    print(indicess)
        ...
        out_l = len(indicess)
        pos_left_up = np.transpose(np.array([np.random.uniform(-half_width, -half_width+1, out_l),np.random.uniform(0, half_len, out_l)]))
        pos_left_down = np.transpose(np.array([np.random.uniform(-half_width, -half_width+1, out_l),np.random.uniform(-half_len, 0, out_l)]))
        left_preference = np.random.choice(a=[0,1], p=[0.8, 0.2], replace=True, size=out_l)
        pos_left_up = [ [x[0]*y, x[1]*y] for x,y in zip(pos_left_up, left_preference)]
        pos_left_down = [ [ -x[0]*(y-1), -x[1]*(y-1)] for x,y in zip(pos_left_down, left_preference)]
        for ind in range(len(pos_left_up)) :
            if (pos_left_up[ind][0] == 0) and (pos_left_up[ind][1] == 0) :
                pos_left_up[ind] = pos_left_down[ind]
        pos_left = pos_left_up
        print(np.shape(pos_left))
        if len(pos_left) == 0:
            pos_left = np.random.normal(1.34, 0.26, size=(out_l, 2))
        # pos_left_x = [ [x[0]] for x in pos_left]
        # pos_left_y = [ [x[1]] for x in pos_left]
        x_vel_left = np.random.normal(1.34, 0.26, size=(out_l, 1))
        print('left')
        print(np.shape(pos_left), np.shape(x_vel_left))
        ...
        for j in range(out_l):
            print(pos_left[j], pos_left[j])
            new_initial[(indicess)[j], 0:1] = pos_left[j][0:1]
        ...
        out_r = len(indicess)
        pos_right_up = np.transpose(np.array([np.random.uniform(half_width, half_width-1, out_r),np.random.uniform(0, half_len, out_r)]))
        pos_right_down = np.transpose(np.array([np.random.uniform(half_width, half_width-1, out_r),np.random.uniform(-half_len, 0, out_r)]))
        right_preference = np.random.choice(a=[0,1], p=[0.2, 0.8], replace=True, size=n)
        pos_right_up = [ [x[0]*y, x[1]*y] for x,y in zip(pos_right_up, right_preference)]
        pos_right_down = [ [ -x[0]*(y-1), -x[1]*(y-1)] for x,y in zip(pos_right_down, right_preference)]
        for ind in range(len(pos_right_up)) :
        ...
        pos_right = pos_right_up
        if len(pos_right) == 0 :
            pos_right = np.random.normal(1.34, 0.26, size=(out_r, 2))
        print('right')

        x_vel_right = np.random.normal(1.34, 0.26, size=(out_r, 1))
        print(np.shape(pos_right),  np.shape(x_vel_right))
        state[indicess, 0:3] = np.concatenate((pos_right, x_vel_right), axis=-1)
         # np.shape(pos_left_x), np.shape(pos_left_y),
...
#    print(total, times, np.mean(times), velocities, np.mean(velocities)) # np.mean(times),, np.mean(velocities)
    ...
    if (visual == True ) :
        with visualize(states, space, 'docs/walkway_{}_{}_{}_preference.gif'.format(mode, n, distance)) as _:
            pass
...
test_walkway_benchmark(50, 6, 30, 'benchmark', 0, True)
...
#     layout_df.to_csv('data/df_{}'.format(layout))
...
# distance = np.arange(2, 16, 2)
# for d in distance :
#     res = test_walkway_benchmark(20, 6, 30, 'pillars', 0, True, distance = d)
